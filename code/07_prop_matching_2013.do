...
forval i = 1/1 {
	cap log close 
	log using "$oo/logs/prop_matching2013year_t`i'.pdf", replace
	* import ACS data 
	use "$oi/working_acs", clear 
...
drop if countyfip==000
	...
	keep statefip countyfip year phat ever_treated_county wt d_1 x_1 d_0 x_0 d_0w x_0w

	compress
	save "$oi/troubleshoot/propensity_weights2013year_t`i'", replace


	log close 
	translate "$oo/logs/prop_matching2013year_t`i'.pdf" "$oo/logs/prop_matching2013year_t`i'.pdf", translator(smcl2pdf) replace
