...
**************************************************************/
use "$oi/propensity_weights2012puma_t2", clear 
twoway (line d_1 x_1 , sort  lpattern(solid) lcolor(midblue) lwidth(0.3)  ) ///
	(line d_0 x_0 , sort lpattern(longdash) lcolor(dkorange) lwidth(0.4)  ) ///
	 ...
	 xtitle("Pr(puma is treated)") ytitle("Density") xsize(7) xlabel(0(0.5)1)
...
collapse (sum) tot_*  (mean) mean_tot_* ,  by(year ever_treated_puma)

foreach v of varlist tot_* mean_tot_* {
	format 	`v' %12.0fc
}

twoway (connected mean_tot_targetpop2 year if ever_treated_puma==1, mcolor(black) lcolor(black)) ///
	(connected mean_tot_targetpop2 year if ever_treated_puma==0, mcolor(gray) lcolor(gray) lpattern(longdash)) ///
	, legend(pos(6) row(1) order(1 "Treated" 2 "Untreated" )) ytitle("Mean puma target population") xtitle("Year") ///
	xlabel(2013(1)2019) ylabel(0(200)600)
graph export "$oo/final/mean_target2_pop.png", replace

twoway (connected tot_targetpop2 year if ever_treated_puma==1, mcolor(black) lcolor(black)) ///
	(connected tot_targetpop2 year if ever_treated_puma==0, mcolor(gray) lcolor(gray) lpattern(longdash)) ///
...
reghdfe log_tot_placebo5 gain_ry_minus4_group gain_ry_minus3 gain_ry_minus2 o.gain_ry_minus1 ///
	gain_ry_plus0 gain_ry_plus1 gain_ry_plus2_group  ///
	lost_ry_minus4_group lost_ry_minus3 lost_ry_minus2 o.lost_ry_minus1 ///
	lost_ry_plus0 lost_ry_plus1 lost_ry_plus2 lost_ry_plus3_group ///
	$covarspop [aw=tot_targetpop2] , ///
...
coefplot ///
	...
	legend(order(4 "Active 287(g)" 2 "No 287(g)") row(1) pos(6)) xsize(6) ///
	ylabel(-.75(0.25).5)
graph export "$oo/final/logtargetpop_gain_estudy_prop.png", replace
...
coefplot ///
	...
	legend(order(2 "Active 287(g)" 4 "No 287(g)") row(1) pos(6)) xsize(6) ///
	ylabel(-.75(0.25).5)
graph export "$oo/final/logtargetpop_lost_estudy_prop.png", replace
