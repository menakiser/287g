...
Though we focus on a population specifically targeted by 287(g) deportations, I also evaluate effects on a placebo population that is not affected by this policy: U.S. born citizens, non-Hispanic, of white race, with similar non-immigration characteristics (males, ages 18-39, with a high school degree or less, not currently married). This group, being U.S.-born citizens, is not at risk of deportation by the 287(g) program and their demographics does not fit the profiles of deportees, thus the risk of profiling if not evident. Evaluating this placebo population allows us to discern if effects in the total population are distinguishable from those that are not affected, in deportation or profiling, by this policy. As a way to evaluate spillover effects, I can also evaluate a population that \textit{should not} be affected by the policy but may be at risk of profiling: U.S. citizens from Hispanic descent, with similar non-immigration characteristics (males, ages 18-39, with a high school degree or less, not currently married), that had either naturalized before the start of the sample or were born in the U.S. I rule out those that may have obtained citizenship during this period as intensified enforcement can prompt some eligible immigrants to naturalize faster as a way to gain protection from deportation \citep{amuedo2020}. This spillover group may decide to relocate away from a 287(g) area, due to hostile attitudes towards Hispanics \citep{baumer_federal_local_2023}, and may be more prone to chose an area that has experience a lost in an agreement when deciding to relocate.  %I'm confused about the distinction between the placebo population and the spillover population. are you thinking us born are completely unaffected, whereas foreign-born citizens might have some spillover? State this more clearly.
    
 \section{Results}

We evaluate this in Table \ref{tab:regpop}, in which we evaluate changes in the size of the target population (Columns 1-2) and placebo population (Columns 3-4) at the puma-year level (Equation \ref{eq:regpop}). In this design, I find that the size of the target population in a puma with an active 287(g) agreement declines by 11.8\% and by 12.6\% when including controls at the puma-year level, both significant at the 1 percent level. Meanwhile, the size of the placebo population decreases at much smaller and statistically insignificant rates: 1.6\% and 2.4\% when including controls. Since we are focusing on a specially narrow target population, these coefficients translate translate to modest absolute changes in population size XX. \citep{parrado_immigration_2012} finds also modest changes in the size of foreign born Mexican population by XX.

%This is an important result and one of your key findings. Say a bit more -- e.g. 12% decline in population seems huge, but need to remember that this is a relatively small targeted population. Are there any prior studies that estimate something similar that you could compare to?
...
\adjustbox{ width=0.7\linewidth}{
    \input{output/final/removaltable}
